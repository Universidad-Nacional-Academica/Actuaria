...
&= 100 \left(\int_{0}^{110-x} v^{t} (1+i_{q})^{\lfloor t \rfloor} {}_tp_x exp \left\{- \int_0^t \mu_{x+u}^{ai} du  \right\} \cfrac{0.01}{exp(0.0t)} dt \right) \\ 
...
#Integral
integrate(ft, 0, n)$value
```
y para el cálculo de la del seguro de vida que paga al final del mes

```{r}
#vida entera pago en la m fracción del año
m = 12
fn = function(t){v^(ceiling(m*t))/m * tpx.perks1(t, x, pars) * muxt.perk1(t, x, pars)}
